...
import { bookshelfEntity, createCustomBookshelfDto, userEntity, userObj } from "tests/fixtures";
...
describe('getMyBookshelves-bookshelf use case tests', () => {
    ...
    test('execute() should return an array of BookshelfEntity', async () => {
        ...
        const result = await new GetMyBookShelves(mockBookshelfRepository, mockUserRepository)
            .execute(userObj.id);
    ...
    test('execute() should throw an error when user with provided ID does not exist', async () => {
        ...
        await expect(new GetMyBookShelves(mockBookshelfRepository,mockUserRepository).execute(userObj.id))
