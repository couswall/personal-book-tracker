...
describe('createCustom-bookshelf.dto test', () => {
    ...
    describe('userId validation', () => {
        test('should throw an error when userId is undefined', () => {
            ...
            expect(error).toBe(BOOK_SHELF_DTO_ERRORS.CREATE_CUSTOM_BOOKSHELF.USER_ID.REQUIRED);
            expect(dto).toBeUndefined();
        });
        test('should throw an error when userId is not a number', () => {
            const dtoObj = {shelfName: 'hello', userId: '1234'} as unknown as ICreateCustomBookShelfDto;
            const [error, dto] = CreateCustomBookShelfDto.create(dtoObj);

            expect(error).toBe(BOOK_SHELF_DTO_ERRORS.CREATE_CUSTOM_BOOKSHELF.USER_ID.NUMBER);
            expect(dto).toBeUndefined();
        });
    });
    describe('shelfName validation', () => {
        test('should throw an error when it is not included', () => {
            ...
            expect(error).toBe(BOOK_SHELF_DTO_ERRORS.CREATE_CUSTOM_BOOKSHELF.SHELF_NAME.REQUIRED);
            expect(dto).toBeUndefined();
        });
        test('should throw an error when it is not a string', () => {
            const dtoObj = {shelfName: [], userId: 1234} as unknown as ICreateCustomBookShelfDto;
            const [error, dto] = CreateCustomBookShelfDto.create(dtoObj);

            expect(error).toBe(BOOK_SHELF_DTO_ERRORS.CREATE_CUSTOM_BOOKSHELF.SHELF_NAME.STRING);
            expect(dto).toBeUndefined();
        });
        test('should throw an error when it contains only blank spaces', () => {
            ...
            expect(error).toBe(BOOK_SHELF_DTO_ERRORS.CREATE_CUSTOM_BOOKSHELF.SHELF_NAME.BLANK_SPACES);
            expect(dto).toBeUndefined();
        });
        test('should throw an error when shelfName length is less than two characters long', () => {
            ...
            expect(error).toBe(BOOK_SHELF_DTO_ERRORS.CREATE_CUSTOM_BOOKSHELF.SHELF_NAME.MIN_LENGTH);
            expect(dto).toBeUndefined();
        });
        test('should throw an error when shelfName length is more than 90 characters long', () => {
            const dtoObj = {...createCustomBookshelfDto, shelfName: 'Bookshelf'.repeat(90)};
            const [error, dto] = CreateCustomBookShelfDto.create(dtoObj);

            expect(error).toBe(BOOK_SHELF_DTO_ERRORS.CREATE_CUSTOM_BOOKSHELF.SHELF_NAME.MAX_LENGTH);
            expect(dto).toBeUndefined();
        });
    });
});